...
def test_disallowed_risk_flags() -> None:
    expected = ExpectedOutcome(expected_decision="apply", disallowed_risk_flags=["AWS experience"])
    bad = score_case(
        expected=expected,
    ...
    if error:
        return EvalRecord(
            ...
            telemetry=CaseTelemetry(
                latency_ms=10.0, calls=[], input_tokens=0, output_tokens=0, estimated_usd=None
            ),
...
def test_aggregate_total_usd_sums_with_unknown_model_safe() -> None:
    ...
    r2 = _record("b", score=80).model_copy(
        update={
            "telemetry": _record("b", score=80).telemetry.model_copy(update={"estimated_usd": None})
        }
    )
    agg = aggregate([r1, r2])
...
def test_aggregate_total_usd_none_when_all_unknown() -> None:
    r1 = _record("a", score=80).model_copy(
        update={
            "telemetry": _record("a", score=80).telemetry.model_copy(update={"estimated_usd": None})
        }
    )
    agg = aggregate([r1])
