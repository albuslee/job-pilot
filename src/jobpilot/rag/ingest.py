...
def ingest_profile_dir(profile_dir: Path, *, store: RagStore) -> int:
    if not profile_dir.is_dir():
        raise FileNotFoundError(f"Profile directory not found: {profile_dir}")
    docx_files = sorted(p for p in profile_dir.iterdir() if p.suffix.lower() == ".docx")
    if not docx_files:
    ...
    for path in docx_files:
        ...
        log.info("ingest.parsed", file=path.name, chunks=len(chunks))
